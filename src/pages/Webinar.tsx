...
const Webinar = () => {
  ...
  const handleRecording = async () => {
    ...
    try {
      await startRecording(roomId, `${currentUser?.name || 'Presenter'} - ${roomId}`)
      ...
      {showEndCallConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl border border-gray-200 bg-white p-5 text-gray-900 shadow-xl dark:border-gray-800 dark:bg-gray-900 dark:text-white">
            <h3 className="text-base font-semibold">End webinar?</h3>
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button onClick={() => { setShowEndCallConfirm(false); handleEndWebinar() }} className="rounded-lg border border-red-600 bg-red-600 px-3 py-1.5 text-sm font-medium text-white">End call</button>
            </div>
          </div>
        </div>
      )}
